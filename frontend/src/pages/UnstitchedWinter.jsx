  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch(
        "https://capital-store-backend.vercel.app/api/products?category=Unstitched&subCategory=Winter"
      );
      const data = await res.json();
      setProducts(data);
    } catch (err) {
      console.error("Failed to fetch unstitched winter products", err);
    } finally {
      setLoading(false);
    }
  };
...
if (sort === "new") {
  ...
  <p className="text-center text-gray-500">
    Loading unstitched winter products...
  </p>
)}

{!loading && filtered.length === 0 && (
  <p className="text-center text-gray-500">
    No unstitched winter products found.
  </p>
)}
