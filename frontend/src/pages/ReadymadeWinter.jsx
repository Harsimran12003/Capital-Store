...
export default function ReadymadeWinterPage() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch(
        "https://capital-store-backend.vercel.app/api/products?category=Readymade&subCategory=Winter"
      );
      const data = await res.json();
      setProducts(data);
    } catch (err) {
      console.error("Failed to fetch readymade winter products", err);
    } finally {
      setLoading(false);
    }
  };
...
if (sort === "new") {
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-5 mb-20">
        ...
        <Breadcrumb category="Readymade Winter" />

        <h1 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8">
          Readymade Winter Wear
        </h1>
  ...
  <p className="text-center text-gray-500">
    Loading readymade winter products...
  </p>
)}

{!loading && filtered.length === 0 && (
  <p className="text-center text-gray-500">
    No readymade winter products found.
  </p>
)}
